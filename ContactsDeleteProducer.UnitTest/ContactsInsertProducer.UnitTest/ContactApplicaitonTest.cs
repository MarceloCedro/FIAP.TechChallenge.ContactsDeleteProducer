using FIAP.TechChallenge.ContactsDeleteProducer.Application.Applications;
...
namespace FIAP.TechChallenge.ContactsDeleteProducer.UnitTest
{
    public class ContactApplicaitonTest
    {
        ...
        private async Task SetupGetContactByIdAsync(bool validReturn)
        {
            if (validReturn)
            {
        ...
        public async Task ValidateFakeTokenAsync()
        {
            ...
            await SetupGetContactByIdAsync(false);

            var retorno = await contactApplication.DeleteContactAsync(null);
        ...
        [Fact]
        public async Task ValidateContactNotFoundAsync()
        {
            await SetupTokenAsync(true);
            await SetupGetContactByIdAsync(false);
            var contactDto = new ContactDeleteDto
            {
                Id = 1
            };
            var retorno = await contactApplication.DeleteContactAsync(contactDto);

            await VerifyTokenAsync(Times.Once());
            await VerifyGetContactByEmailAsync(Times.Once());
            Assert.False(retorno.Success);
            Assert.Equal("Contato nao encontrado na base para delecao.", retorno.Message);
        }
        ...
        {
            await SetupTokenAsync(true);
            await SetupGetContactByIdAsync(true);
            ...
            Assert.False(retorno.Success);
            Assert.Equal("Ocorreu um problema ao tentar inserir o registro na fila para delecao.", retorno.Message);
        }
        ...
        public async Task ValidateContactSendingContactAsync()
        {
            ...
            Assert.Equal("Contato inserido na FILA para delecao com sucesso.", retorno.Message);
        }
